...
def "rng" [] { head -100 /dev/random | b3sum }
...
def "mnt_surv" [] {
  sudo zpool import;
  sudo zpool import surv_pool;
  sudo zfs mount -l surv_pool/enc;
}
def "sk_rg" [] { sk --ansi -i -c 'rg --color=always --line-number \"{}\"' }
def dir_count [] { lsd -la | grep ^d | wc -l }
def ssh_aws [] { ssh -i /home/magewe/aws_tikr_key_pair ubuntu@ec2-3-101-22-91.us-west-1.compute.amazonaws.com  }
